...
-(void)Update:(double)timer
{
	...
	if(m_tEmitter.m_bFirstRun)
	{
		[self FirstRun];
		m_tEmitter.m_bFirstRun = NO;
	}
}
-(void)FirstRun
{
	for(unsigned int i=0;i<m_vctParticles->size();i++)
	{
		(*m_vctParticles)[i].lifeTime = RAND_FLOAT(0.0f, m_tEmitter.minLifetime)/1000.0f;
	}	
}
